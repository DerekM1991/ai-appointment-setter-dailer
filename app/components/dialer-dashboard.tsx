...
type DashboardData = {
  ...
  integrations: {
    ...
    twilio: { configured: boolean; callingLimits: { providerConcurrent: number; providerCps: number; planConcurrent: number; effectiveConcurrent: number; effectiveCps: number; maxAiCallSeconds: number } };
...
function Overview({ data, onNavigate, onImport }: { data: DashboardData; onNavigate: (id: string) => void; onImport: () => void }) {
  const metrics = [
    ...
    { label: "Live sessions", value: `${data.metrics.active} / ${data.integrations.twilio.callingLimits.effectiveConcurrent}`, detail: `${data.workspace.plan.name} + Twilio effective limit`, icon: "phone" as IconName, tone: "blue" },
