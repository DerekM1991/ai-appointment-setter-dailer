...
import { appointments, auditEvents, campaignLeads, campaigns, calls, leads, memberships, organizations, usageCounters } from "@/db/schema";
...
export async function GET() {
  ...
  try {
    ...
    const [eligibleRows, blockedRows, activeRows, bookedRows, leadRows, campaignRows, queueRows, callRows, appointmentRows, auditRows, calendar, connections, memberRows, usageRows, organizationRows] = await Promise.all([
    ...
    return Response.json({
      viewer: { userId: auth.userId, displayName: auth.displayName, email: auth.email, role: auth.role, permissions: EXPOSED_PERMISSIONS.filter((permission) => hasPermission(auth, permission)) },
      workspace: { id: auth.organizationId, name: auth.organizationName, planKey: auth.planKey, plan: auth.plan, subscriptionStatus: auth.subscriptionStatus, trialEndsAt: auth.trialEndsAt, memberCount: Number(memberRows[0]?.value ?? 0), stripeConfigured: config.stripe, hasBillingAccount: Boolean(organizationRows[0]?.stripeCustomerId), currentPeriodEnd: organizationRows[0]?.currentPeriodEnd ?? null, usage: usageRows[0] ?? { periodKey: monthlyPeriodKey(), contactsImported: 0, callsStarted: 0, callMinutes: 0, aiTurns: 0 } },
      metrics: { eligible: Number(eligibleRows[0]?.value ?? 0), blocked: Number(blockedRows[0]?.value ?? 0), active: Number(activeRows[0]?.value ?? 0), booked: Number(bookedRows[0]?.value ?? 0) }, leads: leadRows.map((lead) => ({ ...lead, blockReasons: safeJsonArray(lead.blockReasonsJson), phoneE164: maskPhone(lead.phoneE164) })), campaigns: campaignRows.map((campaign) => ({ ...campaign, queue: queueByCampaign.get(campaign.id) ?? {} })), calls: callRows, appointments: appointmentRows, auditEvents: auditRows.map((event) => ({ ...event, details: safeJsonObject(event.detailsJson) })),
      integrations: { calendar, outlook: { connected: calendar.provider === "microsoft", accountEmail: calendar.provider === "microsoft" ? calendar.accountEmail : null }, google: { connected: calendar.provider === "google", accountEmail: calendar.provider === "google" ? calendar.accountEmail : null }, connections, twilio: { configured: readiness.twilio }, openai: { configured: readiness.openai, model: openaiConnection?.accountIdentifier || runtime.OPENAI_MODEL?.trim() || "gpt-5.6-terra" }, appBaseUrl: { configured: config.baseUrl }, providerReadiness: config }, readiness, readinessPassed: Object.values(readiness).filter(Boolean).length,
    });
