...
import { hasActiveBillingOverride, hasPermission, permissionDenied } from "@/lib/tenant";
...
export async function POST(
  ...
  if (!hasActiveBillingOverride(auth) && ((auth.subscriptionStatus === "past_due" || auth.subscriptionStatus === "canceled") || (auth.planKey === "trial" && auth.trialEndsAt && auth.trialEndsAt < Date.now()))) return Response.json({ error: "An active subscription or platform access grant is required to launch calls." }, { status: 402 });
  try {
    const db = getDb();
